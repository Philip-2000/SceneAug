def parse(): #using pattern to understand scene
    ...
    config_template["phy"]["vis"], config_template["pat"]["vis"] = {"fiv":{"wo":(1.0,0,0),"wi":(0,0,1.0), "dr":(0,1.0,0),}, }, {}#{"pat":True}
    config_template["adjs"] = {"decay":200.0,'inertia':0.0}
    #config_template["pat"]["prerec"] = False
    config_template["phy"]["s4"] = 4
    R = 1.5
    return parser.parse_args(sys.argv[1:]), config_template, sceneLst, R

if __name__ == "__main__": #load="testings",
    args, config, sceneLst, R = parse()
