def parse(): #using pattern to synthesis scene
    ...
    parser.add_argument("-c","--condition", default="roomcond",   choices=["uncond","textcond","roomcond"])
...
if __name__ == "__main__": 
    ...
    from SceneClasses.Operation import gnrt
    P = PM("losy")
    S = scne.fromNpzs(name=lst[0], grp=False, cen=False, wl=True, windoor=True)
    S.OBJES.OBJES.clear()
    G = gnrt(P,S)
    G.roomcond()
    ...
    # from SceneClasses.Operation import patternManager as PM
    # pm = PM(args.version)
    # SDS(name=args.dataset,lst=(lst),prepare=args.condition, grp=False, cen=False, wl=True, windoor=True).synthesis("gnrt",args.condition,pm, use=True, draw=True)